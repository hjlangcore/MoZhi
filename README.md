...
- **🧹 后处理质量管道** — 5步自动清洗：元叙事泄露过滤 → Markdown残留清理 → AI痕迹词降频 → 章节号校验 → 字数范围检查
- **🔒 一致性状态机** — 角色卡锁(不可变) + 里程碑锁定(防重复达成) + 反派状态管理(防无限复活) + 伏笔全生命周期追踪
- **🔄 场景多样性轮换器** — 8种场景池 + FIFO历史队列 + 连续同类警告 + 强制切换（打破"山洞循环"）
- **🏷️ 智能标题去重** — SequenceMatcher+Jaccard 双算法相似度检测 + 4种变换策略自动生成唯一标题
...
| **质量保障** | 后处理管道 + 一致性状态机 + 场景轮换器 + 标题去重（4大防退化模块） |
...
```
每章执行流程：
Phase 0: 质量约束预加载 → 一致性状态机查询 → 场景轮换器选择 → 标题去重检查
Phase 1: 卷上下文注入 → 记忆权重计算 → 推理规划(REASONING) → 联网知识注入
Phase 2: 正文写作(WRITING, 注入反退化Prompt) → 后处理管道清洗(5步)
Phase 3: 6 道门禁检查 → 增强审计(每10章) → 状态回写 + 状态机更新
Phase 4: Agent Loop 动态调度 → 优化/修正/重写 → 下一章

每5章：连贯性检查 + 伏笔管理 + 场景多样性审计
每10章：多维度质量审计 + 趋势分析 + 里程碑一致性校验
每卷结束：自动插入卷标头（第X卷「卷名」）
```

## 🛡️ 质量保障体系

基于 **InkOS 多智能体审计框架** 对 296 章 / 32 万字输出进行全量分析后，构建的 **4 大防退化模块**：

### 问题诊断结果

| 问题类型 | 严重程度 | 检出数量 | 解决方案 |
|---------|---------|---------|---------|
| 剧情原地打转（山洞循环） | 🔴 致命 | 23%章节含"山洞" | 场景轮换器强制切换 |
| 里程碑重复达成 | 🔴 致命 | 突破仙人境 x3 | 一致性状态机锁定 |
| 元叙事泄露（AI指令写入正文） | 🔴 致命 | 第280章等 | 后处理管道过滤 |
| AI痕迹词高频聚集 | 🟠 严重 | 仿佛36次/忽然25次 | AI痕迹词降频器 |
| 角色系统崩坏 | 🟠 严重 | 性别/姓名混乱 | 角色卡锁(frozen) |
| 标题大量重复 | 🟡 中等 | "神秘机关启动"x8 | 标题去重服务 |
...
│       ├── agent_loop.py           # Agent Loop 动态调度引擎（集成质量模块）
│       ├── postprocessor.py        # 🧹 后处理过滤管道（5步清洗）
│       ├── consistency_tracker.py  # 🔒 一致性状态机（角色/里程碑/伏笔）
│       ├── scene_rotator.py        # 🔄 场景多样性轮换器（8种场景池）
...
│       ├── prompts.py              # 专业 Prompt 模板库（含反退化指令）
│       ├── state.py                # 状态模型（含 world_view + consistency 字段）
